...
def potGridTracer(resolution, rMax=20.):
    """
    ...
    radiuses = np.linspace(0., rMax, nBinR+1)
    tracerGrid = [[0., 0., 0.]] # 2D array, each row is a tracer point, in the form of [x, y, z]
    for i in range(len(radiuses)):
        radius = radiuses[i+1]
        perimeter = 2. * np.pi * radius
        nBinTheta = int(perimeter / resolution)
        thetas = np.linspace(0., 2.*np.pi, nBinTheta+1)
        for j in range(len(thetas)):
            theta = thetas[j]
            x = radius * np.cos(theta)
            y = radius * np.sin(theta)
            tracerGrid.append([x, y, 0.])
